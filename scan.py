...
print('[4] gzinflate calls:', stdout.read().decode('utf-8', errors='ignore').strip())

# 5. File count and size
stdin, stdout, stderr = client.exec_command('find /www/wwwroot/resource_site/wp-content/themes/zibll/ -name "*.php" | wc -l')
print('[5] PHP files:', stdout.read().decode('utf-8', errors='ignore').strip())

stdin, stdout, stderr = client.exec_command('du -sh /www/wwwroot/resource_site/wp-content/themes/zibll/')
print('[6] Theme size:', stdout.read().decode('utf-8', errors='ignore').strip())
...
print('[7] Recently modified PHP files:')
...
print('[8] Files with long encoded strings:')
