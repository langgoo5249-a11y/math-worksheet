 ...
 * @LastEditTime: 2024-03-04 00:06:07
...
function follow_action($_name, $ed_name, $_ed_id, $add_msg = '已关注此用户', $rem_msg = '已取消关注此用户')
{
    ...
    $user_meta    = (array) get_user_meta($user_id, $_name, true);
    $ed_user_meta = (array) get_user_meta($_ed_id, $ed_name, true);
    if ($user_meta) {
...
function posts_action($user_meta_name, $post_id, $key, $add_msg = '已完成', $rem_msg = '已取消', $is_comment = false)
{
    ...
    if ($user_id) {
        $user_meta = (array) get_user_meta($user_id, $user_meta_name, true);
        if ($user_meta) {
