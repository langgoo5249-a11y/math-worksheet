 ...
 * @LastEditTime: 2024-05-10 13:07:38
...
function zib_admin_post_posts_custom_column($column_name, $posts_id)
{
    ...
    switch ($column_name) {
        case "pay_data":
            ...
            $con .= '<div style="font-size: 12px;">' . $zibpay_price . ' · 销量' . $sales_volume . ($sales_volume ? '<a href="' . admin_url('admin.php?page=zibpay_order_page&status=1&post_id=' . $posts_id) . '"> [查看]</a>' : '') . '</div>';
            echo '<div>' . $con . '</div>';
            break;
...
function zib_output_users_columns($var, $column_name, $user_id)
{
    ...
    switch ($column_name) {
        ...
        case "assets":
            ...
            $pay_down_log = get_user_meta($user_id, 'pay_down_log', true);
            $html .= $pay_down_log ? '<div class="row-actions px12">' . zibpay_get_paydown_log_admin_link('user', $user_id, "px12", "[会员免费下载记录]") . '</div>' : '';
        ...
        case "all_time":
            ...
            //登录地址
            $addr      = '';
            $addr_data = get_user_meta($user_id, 'user_addr', true);
