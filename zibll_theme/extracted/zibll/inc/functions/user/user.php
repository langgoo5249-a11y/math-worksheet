 ...
 * @LastEditTime: 2024-04-09 19:31:08
...
function zib_updata_user_addr_meta($user_login, $user)
{
    ...
    if (!empty($user_addr['province']) || !empty($user_addr['nation'])) {
        update_user_meta($user->ID, 'user_addr', $user_addr);
    }
}
...
{
    $user_addr = zib_get_remote_ip_addr();
    if ($user_addr) {
        update_user_meta($user_id, 'register_ip', $user_addr);
    }
}
