...
r = subprocess.run(['git', 'add', 'docs/.vitepress/config.ts', 'docs/.vitepress/theme/components/ZiWeiCalculator.vue'], cwd=repo)
r2 = subprocess.run(['git', 'commit', '-m', 'fix(ziwei): add iztro CDN in head and revert wrong window.iztro() call'], cwd=repo, capture_output=True, text=True, encoding='utf-8', errors='replace')
