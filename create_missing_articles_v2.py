...
for item in all_unmatched:
    ...
    url = item['url']
    code = item.get('code', '')
    source = item.get('source', '网盘')
    
    content = f"""<h2>{title}资源介绍</h2>
<p>本资源为<strong>{title}</strong>完整版本，内容详实丰富，适合各类用户学习使用。资源经过精心整理，确保内容完整可用。</p>
...
<h3>适合人群</h3>
<p>本资源适合对相关领域感兴趣的初学者、进阶者以及专业人士使用，无论你是刚入门还是想要深入学习，都能从中获得有价值的内容。</p>

<h3>使用说明</h3>
<p>下载后请按照说明进行操作，如遇到问题可以留言反馈，我们会及时更新修复。建议使用主流播放器或阅读软件打开。</p>

<h3>📥 资源下载</h3>
<p><strong>网盘类型：</strong>{source}</p>
<p><strong>网盘链接：</strong><a href="{url}" target="_blank" rel="noopener">{url}</a></p>"""
...
sftp = ssh.open_sftp()
with sftp.open('/tmp/new_articles.json', 'w') as f:
    f.write(json_data)
sftp.close()

# 创建文章的SQL脚本
sql_script = '''import pymysql, json
from datetime import datetime

with open('/tmp/new_articles.json', 'r') as f:
...
for article in articles:
    ...
    post_name = title.lower().replace(' ', '-').replace('/', '-')
    sql = "INSERT INTO wp_posts (post_author, post_date, post_date_gmt, post_content, post_title, post_status, post_name, post_modified, post_modified_gmt, post_type) VALUES (1, %s, %s, %s, %s, 'publish', %s, %s, %s, 'post')"
    ...
    conn.commit()
    print(f"  [OK] ID={post_id} {title[:40]}")
    created += 1
...
# 写入脚本并执行
with open('/tmp/create_articles.py', 'w') as f:
    f.write(sql_script)

stdin, stdout, stderr = ssh.exec_command('python3 /tmp/create_articles.py', timeout=120)
print("\n=== 创建结果 ===")
...
ssh.exec_command("rm /tmp/new_articles.json /tmp/create_articles.py")
ssh.close()
print("\n全部处理完成！")
