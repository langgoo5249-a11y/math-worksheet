...
r = subprocess.run(['git', 'add', 'docs/.vitepress/theme/components/ZiWeiCalculator.vue'], cwd=repo)
r2 = subprocess.run(['git', 'commit', '-m', 'fix(ziwei): fix isReady always false due to missing iztro() call'], cwd=repo, capture_output=True, text=True, encoding='utf-8', errors='replace')
