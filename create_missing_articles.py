...
for article in articles:
    ...
    if any(k in title for k in ['AI', 'ai', 'DeepSeek', '智能', '人工智能']):
        ...
        cat_id = edu_cat_id
    elif any(k in title for k in ['电影', '视频', '4K', '电影', '漫画', '小说']):
        cat_id = movie_cat_id
    else:
        cat_id = tool_cat_id
    ...
    cursor.execute('''
        ...
        VALUES (1, %s, %s, %s, %s, '', 'publish', 'open', 'open', '', %s, '', '', %s, %s, '', 0, '', 0, 'post', '', 0)
    ''', (now, now, content, title, title.lower().replace(' ', '-'), now, now))
    
    post_id = cursor.lastrowit
...
stdin, stdout, stderr = ssh.exec_command(create_cmd, timeout=120)
print("\n=== 创建结果 ===")
...
ssh.exec_command("rm /tmp/new_articles.json")
ssh.close()
print("\n全部处理完成！")
