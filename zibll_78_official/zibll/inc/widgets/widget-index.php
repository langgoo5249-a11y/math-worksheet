...
//保存数据时候刷新缓存
function zib_widgets_register_pages_update_cache($meta_id, $object_id, $meta_key, $_meta_value)
{
    if ($meta_key == 'widgets_register_container' || $meta_key == 'widgets_register') {
        wp_cache_delete('widgets_register_pages', 'zib_cache_group');
        zib_get_widgets_register_pages();
    }
}
add_action('updated_post_meta', 'zib_widgets_register_pages_update_cache', 10, 4);
add_action("added_post_meta", 'zib_widgets_register_pages_update_cache', 10, 4);
