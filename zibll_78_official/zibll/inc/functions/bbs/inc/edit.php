 ...
 * @LastEditTime: 2024-06-13 19:17:22
...
class zib_bbs_edit
{
    ...
    public static function plate_select_lists_tab($posts_id = 0, $class = '', $con_class = '')
    {
        ...
        //添加热门
        $tab_nav_lists = zib_get_ajax_tab('nav', $followed_tabs, $ajax_args);
        ...
        $tab_con = zib_get_ajax_tab('con', $followed_tabs, $ajax_args);
    ...
    public static function plate_select_lists($posts_id = false)
    {
        ...
        $posts_per_page = 20;
        $plate_query    = self::plate_query($paged, 'views', $posts_per_page, $cat_id, $follow);
        $lists          = '';
        $drawer_attr    = wp_is_mobile() ? ' data-drawer="hide" drawer-selector=".plate-drawer"' : ' data-toggle-class="open" data-target=".plate-drop"';
        $in_plate_id    = $posts_id ? zib_bbs_get_plate_id($posts_id) : false;
    ...
    public static function plate_query($paged = 1, $orderby = 'views', $posts_per_page = 20, $cat_id = 0, $follow_user = 0)
    {
        $paged = $paged ? $paged : 1;
        ...
        if ($follow_user) {
            $follow_plate = get_user_meta($follow_user, 'follow_plate', true);
            if ($follow_plate) {
    ...
    public static function plate_move($posts_id = 0)
    {
        ...
        $con .= '<div class="">';
        $con .= self::plate_select_lists_tab($posts_id, '', 'scroll-y mini-scrollbar muted-box max-vh3 padding-6');
        $con .= '</div>';
        $acton = 'plate_move';
