 ...
 * @LastEditTime: 2024-04-15 17:53:03
        ...
        case 9: //购买积分
            ...
            if ($payment_method === 'card_pass') {
                if (_pz('points_pass_exchange_s')) {
                    ...
                    //卡密查询
                    $card_db = ZibCardPass::get_row(array('card' => $password_card, 'password' => $password_password, 'type' => 'points_exchange'));
        ...
        case 4: //会员开通、升级、续费
            ...
            if ($payment_method === 'card_pass') {
                if (_pz('pay_vip_pass_charge_s')) {
                    ...
                    //卡密查询
                    $recharge_card = zibpay_get_vip_exchange_card($password_card, $password_password);
...
function zibpay_initiate_pay($order_data)
{
    ...
    //价格为0，直接付款
    //目前只有卡密兑换会员、积分，金额才允许为0
    if (!$order_data['order_price'] && $order_data['payment_method'] != 'card_pass') {
        $pay = array(
