 ...
 * @LastEditTime: 2023-11-19 14:54:06
...
function zibpay_ajax_admin_paydown_log()
{
    ...
    $record   = $type === 'user' ? get_user_meta($id, $meta_key, true) : get_post_meta($id, $meta_key, true);
    if ($type === 'user') {
    ...
    if ($record && is_array($record)) {
        ...
        foreach ($record_p as $k => $v) {
            ...
            if ($type === 'user') {
                ...
                $_name  = isset($user->display_name) ? $user->display_name : 'user_id:' . $v['user_id'];
