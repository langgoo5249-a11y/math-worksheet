#!/usr/bin/env python3
"""Auto collector for money-making tutorials - v3"""
...
def is_blacklisted(url):
    """Check full URL and domain against blacklist"""
...
def search_baidu(sess, kw, pg=1):
    ...
    try:
        ...
        sess.headers["User-Agent"] = random.choice(UAS)
        r = sess.get(url, timeout=15)
        if r.status_code != 200:
            return []
        if "\u767e\u5ea6\u5b89\u5168\u9a8c\u8bc1" in r.text or "\u9a8c\u8bc1\u7801" in r.text:
            log.warning("Baidu captcha")
            return []
        ...
        for d in soup.select("#content_left > div"):
            ...
            if a and a.get("href", "").startswith("http"):
                href = a["href"]
                # Skip Baidu's own URLs
                if "baidu.com/link" in href or "baidu.com" in urlparse(href).netloc:
                    pass  # Will try to resolve later
                title = a.get_text(strip=True)
                if len(title) > 6:  # Skip very short titles
                    res.append({"title": title, "url": href, "kw": kw, "src": "bd"})
...
def search_google(sess, kw, pg=1):
    off = (pg - 1) * 10
    url = "https://www.google.com/search?q={}&num=10&start={}&hl=zh-CN&tbs=qdr:y".format(quote(kw), off)
    try:
        sess.headers["User-Agent"] = random.choice(UAS)
        r = sess.get(url, timeout=15)
        if r.status_code != 200:
        ...
        for g in soup.select("div.g"):
            ...
            if a and a.get("href"):
                ...
                title = a.get_text(strip=True)
                if len(title) > 6:
                    res.append({"title": title, "url": href, "kw": kw, "src": "gg"})
        log.info("Google \"{}\" p{}: {} items".format(kw, pg, len(res)))
        return res
    except Exception as e:
        log.debug("Google err: {}".format(e))
        return []
...
def extract(sess, url):
    ...
    try:
        ...
        # Check if final URL is blacklisted
        ...
        if HAS_TRAF:
            try:
                ...
                if text and len(clean) > CFG.get("min_content_length", 800):
        ...
        if len(txt) < CFG.get("min_content_length", 800):
            return None
...
class WP:
    def __init__(self):
        ...
        self.s.headers["User-Agent"] = "AutoCollector/3.0"
    ...
    def _get(self, path, **params):
        try:
            ...
            if r.status_code == 401:
                log.error("Auth FAILED (401)")
                return None
            if r.status_code != 200:
                log.debug("GET {} returned {}".format(path, r.status_code))
                return None
            return r.json()
        except Exception as e:
            log.error("GET {} err: {}".format(path, e))
            return None
    ...
    def _post(self, path, **kwargs):
        try:
            ...
            if r.status_code not in (200, 201):
                log.error("POST {} failed: {} - {}".format(path, r.status_code, r.text[:200]))
                return None
            return r.json()
        except Exception as e:
            log.error("POST {} err: {}".format(path, e))
            return None
    ...
    def cat_id(self, name):
        ...
        if data and "id" in data:
            return data["id"]
        log.warning("Category creation failed, using default")
        return 1
    ...
    def upload_img(self, img_url):
        try:
            ...
            if "image" not in ct:
                return None
            content_len = int(ir.headers.get("Content-Length", 0))
            if content_len > 5 * 1024 * 1024:  # Skip images > 5MB
                return None
...
def main():
    ...
    try:
        ...
        if cid == 1:
            log.warning("Using default category - category creation may have failed")
        ...
        for kw in kws:
            ...
            # Then Google as last resort
            if not items:
                items = search_google(sess, kw)
            if not items:
                log.info("No results for: {}".format(kw))
                continue
            ...
            for it in items:
                ...
                if pid:
                    ...
                    log.warning("Fail publish: {}".format(title[:50]))
